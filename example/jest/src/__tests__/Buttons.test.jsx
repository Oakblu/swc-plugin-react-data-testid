...
describe("IconButton — data-testid generation", () => {
  it("adds testids to button and both spans", () => {
    render(<IconButton icon="★" onClick={jest.fn()}>Star</IconButton>);
  ...
  it("counter is isolated from Button", () => {
    const { container } = render(
      <div>
        <Button>B</Button>
        <IconButton icon="x" onClick={jest.fn()}>I</IconButton>
      </div>
    );

    // Each component has its own counter — IconButton.button is 1, not 2
